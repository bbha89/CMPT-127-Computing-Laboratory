...
void ship_draw( const ship_t* ship )
{
  ...
  */
  int i;
  int j;
  for(i = -1;i<=1;i++)//think of it as 9 squares,bottom is (0,0),(0,1),(1,0),(1,1)
  {
    for(j= -1;j<=1;j++)
...
void roid_draw( const roid_t* roid )
{
  ...
  */
  ...
  while(a<=1)
  {
  	for(b=-1;b<2;b++)
  	{
    draw_rectangle( roid->x+a-roid->width/2.0, roid->y+b-roid->height/2.0, 
      roid->x+a+roid->width/2.0, roid->y+b+roid->height/2,
      roid->color );
    }
  a++;
  }
...
int shot_roid_hit( const shot_t* shot, const roid_t* roid )
{
   ...
   */
	...
	while(k<2)
	{
		...
		for(l = -1;l<2;l++)
		{
	  		if( shot->x+k >= roid->x - roid->width/2 &&
	  		shot->x+k <= roid->x + roid->width/2 &&
	  		shot->y+l >= roid->y - roid->height/2 &&
	  		shot->y+l <= roid->y + roid->height/2 )            
	  		return 1; 			
		}
	k++;
